...
test('throws for non-existent Kubernetes version', async () => {
  await expect(checkKubernetesVersion(dir, inputs)).rejects.toThrow(/not found/);
});

// Bad: test implementation detail
test('calls gitHubRequest with correct URL', async () => {
  expect(github.gitHubRequest).toHaveBeenCalledWith(expect.objectContaining({url: '...'}));
});
```
...
|-------|-------------|--------|
| Phase 1 | Create HTTP test server utility, refactor `github.test.js` to fully behavioral | Not started |
